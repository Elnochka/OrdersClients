...
public class OrderService {
    ...
    public OrderEntity createOrder(CreateOrderRequest req) {
        ...
        Client first = clientRepo.findByIdForUpdate(ids.get(0)).orElseThrow(() ->
                new BusinessException("Client not found"));
        Client second = clientRepo.findByIdForUpdate(ids.get(1)).orElseThrow(() ->
                new BusinessException("Client not found"));
        ...
        BigDecimal supplierCurrent = orderRepo.sumRevenueAsSupplier(supplier)
                .subtract(orderRepo.sumCostAsConsumer(supplier));
        BigDecimal consumerCurrent = orderRepo.sumRevenueAsSupplier(consumer)
                .subtract(orderRepo.sumCostAsConsumer(consumer));
        ...
        if (supplierFuture.compareTo(new BigDecimal("-1000")) < 0 ||
            ...
            throw new BusinessException("Creating this order would make a client's " +
                    "total profit < -1000");
        }
